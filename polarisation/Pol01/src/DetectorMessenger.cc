...
{ 
  ...
  fTargetPolarizationZ = new G4UIcmdWithADoubleAndUnit("/pol/targetPolarizationZ", this);
  fTargetPolarizationZ->SetGuidance("Set Z polarization of target");
  fTargetPolarizationZ->SetParameterName("PolarizationZ", false);
  fTargetPolarizationZ->AvailableForStates(G4State_Idle);
}
...
{ 
  ...
  if (command == fTargetPolarizationZ)
  {
      fDetector->SetTargetPolarizationZ(fTargetPolarizationZ->GetNewDoubleValue(newValue));
  }
}
