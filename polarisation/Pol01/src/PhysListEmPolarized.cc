...
void PhysListEmPolarized::ConstructProcess()
{
  ...
  while( (*particleIterator)() ){
    ...
    if (particleName == "gamma") {
      ...
      G4ePolarizedIonisation* ionisationProcess = new G4ePolarizedIonisation();
      ionisationProcess->SetEmModel(new G4PolarizedMollerBhabhaModel());
      ionisationProcess->SetMaxKinEnergy(12000.); // 12 GeV
      pmanager->AddProcess(ionisationProcess,  -1,1,1);

      pmanager->AddProcess(new G4eMultipleScattering, -1, 2, 2);
      ...
      ionisationProcess->SetEmModel(new G4PolarizedMollerBhabhaModel());
      ionisationProcess->SetMaxKinEnergy(12000.); // 12 GeV
      pmanager->AddProcess(ionisationProcess, -1, 2, 2);
